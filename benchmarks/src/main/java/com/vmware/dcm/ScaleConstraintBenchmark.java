...
public class ScaleConstraintBenchmark {
    ...
    @Param({"Scheduler Constraint, Solver Filter",
            "Scheduler Constraint, No Solver Filter",
            "No Scheduler Constraint, Solver Filter",
            "No Scheduler Constraint, No Solver Filter"})
    ...
    public static class BenchmarkState {
        ...
        public void setUp() {
            ...
            if (filtering.equals("Scheduler Constraint, Solver Filter") ||
                    filtering.equals("Scheduler Constraint, No Solver Filter")) {
                conn.execute("create view filtered_nodes as " +
                        "select name from node_info " +
                        "where name < 'nod';");
                final String constraint = "create view constraint_allowed_nodes as " +
                        "select * from pods_to_assign " +
                        "check controllable__node_name in " +
                        "(select name from filtered_nodes)";
                policies.add(constraint);
            }
    ...
    public void testSinglePodPlacement(final BenchmarkState state) {
        ...
        final Result<? extends Record> solverOutput;
        if (filtering.equals("Scheduler Constraint, Solver Filter") ||
                filtering.equals("No Scheduler Constraint, Solver Filter")) {
            System.out.println("Benchmark running with filtering...");
            solverOutput = state.scheduler.runOneLoop((table) -> {
            ...
            System.out.println("Benchmark running without filtering...");
            solverOutput = state.scheduler.runOneLoop();
        }
