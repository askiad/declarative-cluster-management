...
dcmMetrics <- data.table(dbGetQuery(conn, "select expId, batchId, dcmSolveTime, scopeLatency,
          orToolsWallTime from dcm_metrics"))
...
dcmScopedSchedulerName <- "DCM+AutoScope"
...
params[scheduler == "DCM+AutoScope", Scheme := scheduler]
params <- params[affinityProportion == 100]
...
if (varyFExperiment) {
    ...
    for (nodes in c(500, 5000, 10000)) {
        scopeMetricsPdfPlot
        savePlot(scopeMetricsPdfPlot, "plots/scopeMetricsPdfPlotVaryNF=",
                 scopeMetricsWithParams$numNodes[1], params, plotHeight/2, plotWidth*4/3, scale=FALSE)    
        scopeMetricsPdfPlot <- applyTheme(
            ggplot(scopeMetricsWithParams[numNodes == nodes],
                   aes(x = scopePercent), size = 1) +
            geom_density(alpha = 0.4, fill = "#7fc97f") + 
            xlab("Scoped nodes percent (%)") +
            ylab("PDF") +
            labs(col = "", linetype = "") 
        ) + guides(col = guide_legend(nrow = 1), linetype=guide_legend(keywidth = 1.6, nrow=2, byrow=TRUE))
    ...
    scopeMetricsPdfPlot
    savePlot(scopeMetricsPdfPlot, paste("plots/scopeMetricsPdfPlotVaryN=", nodes, sep=""),
             scopeMetricsWithParams$numNodes[1], params, plotHeight/2, plotWidth*4/9, scale=FALSE)
    }
}

if (varyFExperiment) {
    medianDCM <- median(perPodSchedulingLatencyWithParams[scheduler=="DCM"]$Latency
                  / 1e6 / perPodSchedulingLatencyWithParams[scheduler=="DCM"]$BatchSize)
    medianAutoScope <- median(perPodSchedulingLatencyWithParams[scheduler=="DCM+AutoScope"]$Latency
                  / 1e6 / perPodSchedulingLatencyWithParams[scheduler=="DCM+AutoScope"]$BatchSize)
    ...
    schedulingLatencyEcdf100xPlot <- applyTheme(
        ...
        scale_x_log10(breaks=c(1, 10, 100, round(medianDCM, 2), round(medianAutoScope, 2))) +
        geom_vline(aes(xintercept=median(perPodSchedulingLatencyWithParams[scheduler=="DCM"]
                           $Latency / 1e6 / BatchSize)), linetype="dashed", size=0.5) +
        geom_vline(aes(xintercept=median(perPodSchedulingLatencyWithParams[scheduler=="DCM+AutoScope"]
                           $Latency / 1e6 / BatchSize)), linetype="dashed", size=0.5) +
        facet_grid(factor(paste("InterPodAffinity=", affinityProportion, "%", sep=""), 
                          levels = c("InterPodAffinity=0%", "InterPodAffinity=50%", "InterPodAffinity=100%")) ~ . ) +
        ## coord_cartesian(xlim = c(1.1, 100)) +

        xlab("Scheduling Latency (ms)") +
        ylab("ECDF") +
        labs(col = "", linetype = "") 
    ) + guides(col = guide_legend(nrow = 1), linetype=guide_legend(keywidth = 1.6, nrow=1, byrow=TRUE))
...
if (varyFExperiment) {
    ...
    dcmLatencyBreakdownPlot <- applyTheme(
        ggplot(longDcmLatencyMetricsWithBatchSizes[batchId >= batchIdMin]) +
        stat_ecdf(size=1, aes(x=value/1e6/N, col=variableClean, linetype=variableClean)) + 
        coord_cartesian(xlim = c(0.01, 110)) +
        scale_x_log10(breaks=c(0.01, 0.1, 1, 10, 50)) +
        xlab("Latency (ms)") +
        ylab("ECDF") +
        facet_grid(scheduler ~ factor(paste("N=", numNodes, sep=""),
                          levels=sapply(params$numNodes[1:3],
                                        function(n) paste(c("N=", n), collapse = "")))) +
        guides(linetype=guide_legend(keywidth = 2, keyheight = 1),
               colour=guide_legend(nrow=1, keywidth = 2, keyheight = 1))
    ) + theme(legend.title=element_blank())

    dcmLatencyBreakdownPlot
    savePlot(dcmLatencyBreakdownPlot, "plots/dcmLatencyBreakdownVaryNF=",
             longDcmLatencyMetricsWithBatchSizes$affinityProportion[1], params, plotHeight, plotWidth*4/3, scale=FALSE)
}
