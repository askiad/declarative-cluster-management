...
public class KubernetesLocalExpr {
    ...
    private int podsPerPriorityLevel = 5;
    private int pr1StartTimeSec = 0;
    private int pr2StartTimeSec = 4;
    private int pr2EndTimeSec = 30;
    private int pr1EndTimeSec = 40;
    ...
    public void run(final NamespacedKubernetesClient client, final String configFileName, final IPodDeployer deployer)
        ...
        try (final InputStream inputStream = getClass().getClassLoader().getResourceAsStream(configFileName)) {
            ...
            podsPerPriorityLevel = (int) config.get("podsPerPriorityLevel");
            pr1StartTimeSec = (int) config.get("pr1StartTimeSec");
            pr2StartTimeSec = (int) config.get("pr2StartTimeSec");
            pr2EndTimeSec = (int) config.get("pr2EndTimeSec");
            pr1EndTimeSec = (int) config.get("pr1EndTimeSec");
            waitTimeSec = (int) config.get("waitTimeSec");


    final List<Pod> deployment1 = getDeployment(client, "priority-level1");
            ...
            final List<Pod> deployment2 = getDeployment(client, "priority-level2");
            // create deployment in the k8s cluster at the correct start time
